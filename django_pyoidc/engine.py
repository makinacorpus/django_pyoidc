...
class OIDCEngine:
    ...
    def call_get_user_function(self, client: OIDCClient, tokens: Optional[Dict[str, Any]] = None) -> Any:
        if tokens is None:
            tokens = {}
    ...
    def introspect_access_token(self, access_token_jwt: Optional[str], client: OIDCClient) -> Any:
        """
        ...
        if access_token_jwt is None:
            msg = "Nothing in access_token_jwt."
    ...
    def _call_introspection(self, access_token_jwt: str, client: OIDCClient) -> MutableMapping[str, Union[str, bool]]:
        cache_key = self.general_cache_backend.generate_hashed_cache_key(access_token_jwt)
        try:
    ...
    def call_validate_tokens_hook(self, access_token_jwt: str, client: OIDCClient) -> Any:
        if self.opsettings.get("hook_validate_access_token") is not None:
            logger.debug("OIDC, Calling hook_validate_access_token.")
            return self.call_function("hook_validate_access_token", access_token_jwt, client)
        else:
            logger.debug(
